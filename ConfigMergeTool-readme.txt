...
Format:
  base_relative_path=release_relative_path

  One mapping per line.  Lines starting with # are ignored.
  Paths may be relative to the working directory or to their respective dirs.
...
Format:
  One relative file path per line (relative to working directory).
...
Fields:
  ...
  mapping_file   (optional)  Per-node mapping file; overrides the global one
...
-----------------------------------------------------------
  ...
  For each key in base:
    - If key exists in release → output gets BASE value.
  ...
  Commented-out keys in release (# key=value) are uncommented if base has a
  value for that key.
...
File sections (right panel):
  ...
  Legend bar (bottom of screen — always visible):
    Colour meanings are shown in a fixed bar at the bottom of the browser
    window regardless of scroll position.
    Hover over any legend item to see a full explanation.
...
================================================================================
  ...
  DUPLICATE_KEY                  Duplicate key in base file — last value used
